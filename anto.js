...
Total Pengguna yang telah terdaftar ${pendaftar.length}`)
    ...
    /*      arg = body.trim().split(' ')
          if (arg.length >= 0) {
            ...
            //const rjh2 = `*Hasil Pengamatan!*\nPasangan dengan nama ${kamu} dan ${pacar}\n\n➸ Cinta : ${rpmn}\n➸ Jodoh : ${rpmn2}\n➸ Kemiripan : ${rpmn3}\n➸ Kesukaan : ${rpmn4}\n➸ Kesamaan : ${rpmn5}\n➸ Kebucinan ${rpmn6}`
           await anto.reply(from, `[ *INTROGASI SUKSES* ]\n*[Nama]:${namao}*\n*[Watak]:${wtk}*\n*[Akhlak✨]:${akhlak}*\n*[Sifat]:${sft}*\n*[Hobby]:${hby}*\n*[Tipe]:${typo}*\n*[Kelebihan]:${klbh}*\n*Note*\n_ini hanya main main_`, id)
...
╚════════════════════`
   ...
   /*     case '!igstalk':
            ...
            const stalk = await get.get(`http://melodicxt.herokuapp.com/api/igprofile?user=${args[1]}&apiKey=administrator`).json()
            ...
            const caps = `➸ *Nama* : ${Name}\n➸ *Username* : ${Username}\n➸ *Jumlah umatnya* : ${Jumlah_Followers}\n➸ *Jumlah Yang di sembah* : ${Jumlah_Following}\n➸ *Jumlah status* : ${Jumlah_Post}\n➸ *Biodata* : ${Biodata}`
       ...
       /* case '!getses':  if(isReg(obj)) return
        ...
        case '!jadwaltvnow':
          ...
          case '!waifu':
            ...
            anto.sendFileFromUrl(from, wolt.data.result, 'water.jpg', id)
            break
            case '!pornhub':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
        ...
        case '!loli':
           ...
           // await limitAdd(serial)
            ...
            try {
              ...
              const {judul, album, mp3, thumb, dipublikasi} = music2.data.result[0]
              const musik = `╔════════════════════
║╭──❉[ *[Complete Search]* ]❉──
║│++ 
║│+ ➸ *[Judul]:* ${judul}
...
╚════════════════════`
              ...
              const base64 = `data:image/jpg;base64,${pictk.toString("base64")}`
              anto.sendImage(from, base64, judul, musik)
        ...
        case '!logoninja':
            ...
            if (argz.length >= 3) {
                ...
                anto.sendFileFromUrl(from, `https://tobz-api.herokuapp.com/api/textpro?theme=ninjalogo&text1=${ninja1}&text2=${ninja2}`)
            //  await limitAdd(serial)
            } else {
                await anto.reply(from, `Wrong Format!\n[❗] Kirim perintah *!ninja  |Teks1|Teks2*, contoh *!ninja |Anto|Bot*`, id)
            }
            break
            /*case '!bpink':
            case '!blackpink':
          ...
          /*  case '!ph':
            ...
            case '!neon':
             ...
             const neon = await axios.get(`https://turu-rest.herokuapp.com/neon?text=${text7}`)
              .then(result => {
              anto.sendFilefromUrl(from, result.data.result, 'neon.jpg', '', id)
                }).catch(err => {
                return console.error(err)
                 })
                break
            ...
            if (args.length == 0) return anto.reply(from, 'Contoh: !glow text', id)
             ...
             const glow = await axios.get(`https://turu-rest.herokuapp.com/glow?text=${body.slice(5)}`)
              .then(result => {
              anto.sendFilefromUrl(from, result.data.result, 'glow.jpg', '', id)
                }).catch(err => {
                return console.error(err)
                 })
                break
            case '!glitch':
             if (args.length > 1) return
             const tewq1 = arg.split(' | ')[0]
             const tewq2 = arg.split(' | ')[1]
             const glitch = await axios.get(`https://turu-rest.herokuapp.com/glitch?text1=${tewq1}&text2=${tewq2}`)
              .then(result => {
              anto.sendFilefromUrl(from, result.data.result, 'glitch.jpg', '', id)
                }).catch(err => {
                return console.error(err)
                 })
                break
